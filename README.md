...
Standard segmentation analysis treats per-segment estimates as equally reliable regardless of measurement error and how much data underlies them. A 75% churn rate from 17 customers is reported with the same confidence as a 51% rate from 5,349. This project implements a Bayesian hierarchical model with partial pooling as a principled remedy. Segment-level parameters are treated as draws from a shared population distribution, so that small segments are regularized toward the cross-segment mean in proportion to their uncertainty and large segments are left where their data puts them. The degree of pooling is governed by a single estimated hyperparameter $\tau$ rather than asserted by the analyst. The result is that uncertainty is put to productive use in the persuit of better decision-making.

The main analysis uses customer churn data from the [UCI Online Retail II dataset](https://archive.ics.uci.edu/dataset/502/online+retail+ii) and compares partial pooling against no-pooling and full-pooling baselines using leave-one-out cross-validation. The methodology generalizes to any setting where a binary outcome (e.g., customer churn, credit default, lending decisions, programme uptake) is observed across segments with unequal data density. Examples include retail banking portfolios, B2B distributor networks, regional economic indicators, or multi-site program evaluations. Use cases briefly discussed below include credit risk and country-level policy evaluation, the latter being developed in more detail as the small-sample problem is structurally severe in institutional settings where few geographic units and high within-unit variance are the norm.
...
**The problem is structurally identical across domains.** In **credit risk**, a portfolio of borrowers partitioned by industry or region will have thin cells in niche sectors or sparsely served geographies. No-pooling overfits those cells and partial pooling regularizes them. In **multi-site program evaluations** the problem is often most severe. Small countries (e.g., Luxembourg, Malta, Slovenia) produce extreme per-unit estimates driven by one or two projects in a given year, while large countries (e.g., the UK, Germany, France) are well-identified by data alone. Ad hoc remedies such as winsorization or country exclusion resolve the symptom without modelling the underlying uncertainty. Partial pooling resolves the uncertainty structurally: small-country estimates are shrunk toward the cross-country mean in proportion to their uncertainty, with the degree of shrinkage governed by $\tau$ estimated from the full panel rather than asserted by the analyst. The same logic extends to any nested evaluation design where the hierarchical structure is substantively meaningful and data density varies across levels. For instance, a development bank or evaluator estimating country-level program effects across EU member states faces nested data such as firms within regions, regions within countries, and countries within program vintages.
