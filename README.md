...
Partial pooling over heterogeneous customer segments using a Bayesian hierarchical model, with a three-way comparison against no-pooling and full-pooling baselines. The data are from the [UCI Online Retail II dataset](https://archive.ics.uci.edu/dataset/502/online+retail+ii). The methodology generalizes directly to any setting where a binary outcome (e.g., customer churn, credit default, lending decisions, program uptake) is observed across segments with unequal data density. Use cases discussed below include customer analytics, credit risk, and country-level policy evaluation, the latter being developed in more detail because the small-sample problem is structurally severe in this setting. But in reality the scope is much larger still, inclusing retail banking portfolios, B2B distributor networks, regional economic indicators, and any multi-site program evaluations.
